...
module.exports = {
  ...
  forbidden: [
    ...
    },
    {
      ...
      to: { path: "^src/modules/knowledge-modeling/", pathNot: "^src/modules/knowledge-modeling/index\\.ts$" },
    },
    {
    ...
    },
    {
      ...
      severity: "error",
      comment: "Business contexts must not depend on each other in a cycle (would block a future split).",
      from: { path: "^src/modules/(extraction|ingestion|knowledge-modeling|processing)/" },
      to: { path: "^src/modules/(extraction|ingestion|knowledge-modeling|processing)/", circular: true },
    },
    {
