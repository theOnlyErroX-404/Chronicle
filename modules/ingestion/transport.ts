...
const pinnedLookup = (addresses: LookupAddress[]): LookupFunction => (hostname, options, callback) => {
  ...
  if (!first) {
    callback(new Error(`No pinned address available for ${hostname}.`) as NodeJS.ErrnoException, "");
    return;
  }
