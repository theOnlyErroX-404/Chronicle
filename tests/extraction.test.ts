...
const relationship = (source: string, target: string, type: ExtractedRelationship["type"] = "uses"): ExtractedRelationship => ({
  source,
  target,
  type,
  confidence: 1,
...
describe("extractCandidates", () => {
  ...
  it("merges a repeated entity across chunks through an alias", async () => {
    ...
    const c = client({ extractEntities: vi.fn().mockImplementation(async () => calls.shift() ?? []) });
    const merged = await extractCandidates("First sentence. Second sentence.", c, { maxChars: 15 });
    expect(merged.entities).toHaveLength(1);
    expect(merged.entities[0].name).toBe("APT41");
  ...
  it("backs off for a full minute window on rate limits instead of the sub-second retry loop", async () => {
    vi.useFakeTimers();
    try {
      const c = client({ extractEntities: vi.fn(async () => Promise.reject(new ChronicleError("rate limited", 429))) });
