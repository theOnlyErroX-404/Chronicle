...
const relationshipItem = (names: string[]) => ({
  ...
  properties: {
    ...
    source:
      names.length <= MAX_ENUM_ENDPOINTS ? { type: 'string', enum: names } : { type: 'string' },
    target:
      names.length <= MAX_ENUM_ENDPOINTS ? { type: 'string', enum: names } : { type: 'string' },
...
const RelationshipsOnlySchema = z.object({
...
const runEntityPass = async (chat: ChatFn, chunk: string): Promise<ExtractedEntity[]> => {
  const format: ChatFormat =
    config.extractionFormat === 'schema' ? { name: 'entities', schema: entitiesSchema } : 'json';
  return parsePass(
    EntitiesOnlySchema,
    await chat(
      [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: entitiesGuidance(chunk) },
      ],
      format,
    ),
  ).entities;
...
const runRelationshipPass = async (
  chat: ChatFn,
  chunk: string,
  entities: ExtractedEntity[],
): Promise<ExtractedRelationship[]> => {
  ...
  const format: ChatFormat =
    config.extractionFormat === 'schema'
      ? { name: 'relationships', schema: relationshipsSchema(names) }
      : 'json';
  return parsePass(
    RelationshipsOnlySchema,
    await chat(
      [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: relationshipsGuidance(chunk, entities) },
      ],
      format,
    ),
  ).relationships;
...
abstract class BaseLlmClient implements LlmClient {
  ...
  async extractRelationships(
    ...
    const result = await runRelationshipPass(
      (messages, format) => this.chat(messages, format),
      chunk,
      entities,
    );
    llmCache.set(key, result);
...
const timeoutError = () =>
  new ChronicleError(
    ...
    504,
    'https://chronicle.local/problems/llm-timeout',
  );
...
export class OllamaLlmClient extends BaseLlmClient {
    ...
    try {
      response = await fetch(`${ollamaBase()}/api/chat`, {
        ...
        body: JSON.stringify({
          ...
          options: {
            temperature: 0,
            num_predict: config.extractionMaxTokens,
  ...
  async checkHealth(): Promise<void> {
    ...
    const available =
      payload.models?.some((model) => model.name === config.ollamaChatModel) ?? false;
    if (!available) {
      throw new ChronicleError(
        `The configured model '${config.ollamaChatModel}' is not installed. Run: ollama pull ${config.ollamaChatModel}`,
        503,
        'https://chronicle.local/problems/llm-model-missing',
      );
    }
  }
}
...
const classifyOpenAiStatus = (status: number): ChronicleError => {
  if (status === 401 || status === 403) {
    return new ChronicleError(
      'The LLM API key was rejected. Check OPENAI_API_KEY.',
      401,
      'https://chronicle.local/problems/llm-auth',
    );
  }
  if (status === 429) {
    return new ChronicleError(
      'The LLM provider rate limit was reached. Free tiers are capped per minute and per day.',
      429,
      'https://chronicle.local/problems/llm-rate-limit',
    );
  }
  return new ChronicleError(
    `The LLM provider returned HTTP ${status}.`,
    502,
    'https://chronicle.local/problems/llm-upstream',
  );
};
...
export class OpenAiCompatibleLlmClient extends BaseLlmClient {
  ...
  protected async chat(
    ...
    try {
      ...
      if (isTimeout(error)) throw timeoutError();
      throw new ChronicleError(
        'The LLM provider could not be reached. Check OPENAI_BASE_URL.',
        502,
        'https://chronicle.local/problems/llm-upstream',
      );
    }
...
export class FailoverLlmClient implements LlmClient {
  ...
  private async tryEndpoints<T>(
    operation: (client: OpenAiCompatibleLlmClient) => Promise<T>,
  ): Promise<T> {
    ...
    throw new ChronicleError(
      'Every LLM endpoint is cooling down after repeated failures. Try again shortly.',
      503,
      'https://chronicle.local/problems/llm-unavailable',
    );
  }
...
export const getLlmClient = (): LlmClient => {
  ...
  if (config.llmProvider === 'openai') {
    ...
    if (config.openAiEndpoints.length === 1)
      return new OpenAiCompatibleLlmClient(config.openAiEndpoints[0]);
