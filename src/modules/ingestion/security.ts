...
export const resolveSafePublicUrl = async (
  rawUrl: string,
  ...
  const rejectUnsafe = () => {
    throw new ChronicleError(
      ...
      400,
      'https://chronicle.local/problems/unsafe-url',
    );
  };
  ...
  if (
    !addresses.length ||
    addresses.some(({ address, family: resolvedFamily }) =>
      resolvedFamily === 4 ? isPrivateIpv4(address) : isPrivateIpv6(address),
    )
  )
    rejectUnsafe();
