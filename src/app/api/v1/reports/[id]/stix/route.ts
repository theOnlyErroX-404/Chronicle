...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!report.stixBundle)
      throw new ChronicleError(
        'The STIX bundle is not ready yet.',
        409,
        'https://chronicle.local/problems/not-ready',
      );
    return Response.json(report.stixBundle, {
      headers: { 'content-disposition': `attachment; filename=chronicle-${report.id}.stix.json` },
    });
  } catch (error) {
