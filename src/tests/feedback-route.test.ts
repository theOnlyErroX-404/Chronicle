...
describe("POST /api/v1/reports/[id]/feedback", () => {
  it("returns 404 for an unknown report", async () => {
    mockReports.clear();
    const response = await POST(post({ targetType: "entity", targetId: "n1", action: "accept" }), { params: Promise.resolve({ id: "missing" }) });
  ...
  it("returns 409 when the report has no graph yet", async () => {
    ...
    const response = await POST(post({ targetType: "entity", targetId: "n1", action: "accept" }), { params: Promise.resolve({ id: "r1" }) });
  ...
  it("rejects malformed JSON with 400", async () => {
    ...
    const response = await POST(new Request(base, { method: "POST", headers: { "content-type": "application/json" }, body: "{not json" }), { params: Promise.resolve({ id: "r1" }) });
  ...
  it("rejects a body missing required fields with 400", async () => {
    mockReports.clear();
    mockReports.set("r1", baseReport());
    const response = await POST(post({ targetType: "entity" }), { params: Promise.resolve({ id: "r1" }) });
    expect(response.status).toBe(400);
  });
  ...
  it("rejects an invalid action with 400", async () => {
    mockReports.clear();
    mockReports.set("r1", baseReport());
    const response = await POST(post({ targetType: "entity", targetId: "n1", action: "approve" }), { params: Promise.resolve({ id: "r1" }) });
    expect(response.status).toBe(400);
  });
  ...
  it("rejects a correction whose target does not exist in the graph with 422", async () => {
    mockReports.clear();
    mockReports.set("r1", baseReport());
    const response = await POST(post({ targetType: "entity", targetId: "ghost", action: "reject" }), { params: Promise.resolve({ id: "r1" }) });
  ...
  it("accepts a valid correction and appends it", async () => {
    mockReports.clear();
    mockReports.set("r1", baseReport());
    const response = await POST(post({ targetType: "entity", targetId: "n1", action: "correct", correctedValue: { name: "Midnight Blizzard" }, note: "canonical name" }), { params: Promise.resolve({ id: "r1" }) });
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.count).toBe(1);
    expect(body.feedback[0]).toMatchObject({ targetType: "entity", targetId: "n1", action: "correct", correctedValue: { name: "Midnight Blizzard" }, note: "canonical name" });
  ...
  it("appends to existing feedback instead of replacing it", async () => {
    mockReports.clear();
    mockReports.set("r1", baseReport({ feedback: [{ id: "c1", targetType: "entity", targetId: "n1", action: "accept", createdAt: "2026-08-05T00:00:00.000Z" }] }));
    const response = await POST(post({ targetType: "relationship", targetId: "e1", action: "reject" }), { params: Promise.resolve({ id: "r1" }) });
    expect(response.status).toBe(200);
    const body = await response.json();
