...
import { buildGraph, buildStixLiteBundle, completeEntityEndpoints, inferEndpointType } from "@/modules/knowledge-modeling";
...
describe("completeEntityEndpoints", () => {
  ...
  it("does not synthesize a duplicate entity when a relationship cites an alias", () => {
    const completed = completeEntityEndpoints({
      entities: [{ type: "threat-actor", name: "Midnight Blizzard", aliases: ["Cozy Bear"], confidence: 1, evidence: "actor" }],
      relationships: [{ source: "Cozy Bear", target: "evil.example.net", type: "communicates-with", confidence: 1, evidence: "phoned the domain" }],
    });
...
describe("STIX bundle shape", () => {
  ...
  const graph = buildGraph({
    entities: [
      ...
      { type: "vulnerability", name: "CVE-2021-44228", confidence: 0.7, evidence: "CVE-2021-44228" },
      { type: "campaign", name: "Operation Restless", confidence: 0.6, evidence: "Operation Restless" },
    ],
    relationships: [
      { source: "APT41", target: "EvilBoat", type: "uses", confidence: 0.85, evidence: "uses EvilBoat" },
      { source: "APT41", target: "Operation Restless", type: "attributed-to", confidence: 0.6, evidence: "attributed to" },
    ],
  });
  ...
  it("maps sector and country entities to the STIX identity SDO", () => {
    ...
    const objects = bundle.objects as unknown as Array<Record<string, unknown> & { type: string; name: string }>;
