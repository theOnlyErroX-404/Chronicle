...
const resolve = (rawUrl: string, resolver?: (hostname: string) => Promise<Array<{ address: string; family: number }>>) =>
...
describe("resolveSafePublicUrl", () => {
  ...
  it("rejects URLs with embedded credentials", async () => {
    await expect(resolve("https://user:pass@example.com/a")).rejects.toThrow("Only unauthenticated HTTP(S)");
  });
  ...
  it("accepts a public host", async () => {
    await expect(resolve("https://example.com/report", publicIpResolver)).resolves.toBeInstanceOf(URL);
  });

  it("rejects a hostname that resolves to a private address", async () => {
    await expect(resolve("https://intranet.example/report", stubResolver("10.1.2.3"))).rejects.toThrow(/not allowed/);
    await expect(resolve("https://intranet.example/report", stubResolver("192.168.1.1"))).rejects.toThrow(/not allowed/);
    await expect(resolve("https://intranet.example/report", stubResolver("fe80::1"))).rejects.toThrow(/not allowed/);
  });

  it("rejects a host that cannot be resolved", async () => {
    await expect(resolve("https://missing.example/report", async () => { throw new Error("no such host"); })).rejects.toThrow("could not be resolved");
  });
});
