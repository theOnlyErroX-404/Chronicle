...
describe("PDF ingestion (worker-thread isolation)", () => {
  ...
  it("rejects bytes that are not a PDF header", async () => {
    await expect(ingestReport({ kind: "pdf", filename: "fake.pdf", bytes: new TextEncoder().encode("not a pdf at all") })).rejects.toThrow(
  ...
  it("rejects a fake header whose body fails to parse with 422", async () => {
    ...
    await expect(ingestReport({ kind: "pdf", filename: "junk.pdf", bytes: junk })).rejects.toMatchObject({
      name: "ChronicleError",
      status: 422,
