...
const entity = (type: ExtractedEntity['type'], name: string): ExtractedEntity => ({
  type,
  name,
  confidence: 0.9,
  evidence: '',
...
describe('evaluation scoring', () => {
  ...
  it('normalizes punctuation, spacing, and case so CVE names match', () => {
    ...
    const score = scoreEntities(
      [entity('vulnerability', 'CVE 2023 23397')],
  ...
  it("ignores a leading 'The ' when matching entity names", () => {
    ...
    const score = scoreEntities(
      [entity('threat-actor', 'The Lazarus Group')],
  ...
  it('matches a golden entity via an alias', () => {
    const aliases = buildAliasMap([
      { type: 'threat-actor', name: 'Midnight Blizzard', aliases: ['Cozy Bear'] },
    ]);
    const score = scoreEntities(
      [entity('threat-actor', 'Cozy Bear')],
      [{ type: 'threat-actor', name: 'Midnight Blizzard' }],
      aliases,
    );
    expect(score.matched).toBe(1);
  });
  ...
  it('resolves relationship endpoints through aliases', () => {
    const aliases = buildAliasMap([
      { type: 'threat-actor', name: 'Midnight Blizzard', aliases: ['APT29'] },
      { type: 'malware', name: 'SLUI' },
    ]);
  ...
  it('scoreReport computes separate entity and relationship scores', () => {
    ...
    const extracted: ExtractionResult = {
      entities: [
        entity('threat-actor', 'Midnight Blizzard'),
        entity('malware', 'SLUI'),
        entity('country', 'Ukraine'),
      ],
  ...
  it('evaluate aggregates entity and relationship totals separately across reports', async () => {
    ...
    const result = await evaluate([first, second], async (text) => {
      if (text === first.text) {
        return {
          entities: first.entities.map((e) => entity(e.type, e.name)),
          relationships: first.relationships.map((r) => relationship(r.source, r.type, r.target)),
        };
      }
    ...
    expect(result.relationships.expected).toBe(
      first.relationships.length + second.relationships.length,
    );
