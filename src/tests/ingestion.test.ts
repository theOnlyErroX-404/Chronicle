...
const response = (body: string | Uint8Array<ArrayBuffer>, contentType: string, status = 200, extraHeaders: Record<string, string> = {}) =>
...
describe("URL report ingestion", () => {
  ...
  it("fetches a plain-text report", async () => {
    const body = "APT29 used SLUI in Ukraine. The campaign targeted NATO member states and exploited a zero-day in Outlook 2026.";
  ...
  it("rejects a URL whose content type is neither text nor PDF with 415", async () => {
    ...
    await expect(ingestReport({ kind: "url", url: "https://example.com/report.bin" })).rejects.toMatchObject({ status: 415 });
  });
  ...
  it("classifies a fetch failure as 502", async () => {
    ...
    await expect(ingestReport({ kind: "url", url: "https://example.com/report.txt" })).rejects.toMatchObject({ status: 502 });
  });
  ...
  it("follows a redirect and rejects when the redirect cap is exceeded", async () => {
    vi.mocked(fetchPinned).mockResolvedValue(response("", "text/plain", 302, { location: "https://example.com/next" }));
    await expect(ingestReport({ kind: "url", url: "https://example.com/report.txt" })).rejects.toThrow("redirected too many times");
  });

  it("enforces the size limit from the content-length header with 413", async () => {
    vi.mocked(fetchPinned).mockResolvedValue(response("x", "text/plain", 200, { "content-length": String(1024 * 1024 * 1024) }));
    await expect(ingestReport({ kind: "url", url: "https://example.com/huge.txt" })).rejects.toMatchObject({ status: 413 });
  });
...
describe("text normalization", () => {
  ...
  it("raises a 422 (not a 500) for reports with too little extractable text", () => {
    try {
      ...
      expect(error).toMatchObject({ status: 422, type: "https://chronicle.local/problems/insufficient-text" });
    }
  });
});
