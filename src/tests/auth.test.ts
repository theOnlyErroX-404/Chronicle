...
const request = (authorization?: string) =>
  new Request("http://chronicle.local/api/v1/reports", { headers: authorization ? { authorization } : undefined });
...
describe("requireApiToken", () => {
  ...
  it("rejects a missing Authorization header with 401", () => {
    ...
    expect(caught(() => requireApiToken(request()))).toMatchObject({ status: 401, type: "https://chronicle.local/problems/unauthorized" });
  });
  ...
  it("rejects a header that is not a bearer token with 401", () => {
    mockConfig.apiToken = "secret";
    expect(caught(() => requireApiToken(request("Basic dXNlcjpwYXNz")))).toMatchObject({ status: 401 });
  });
