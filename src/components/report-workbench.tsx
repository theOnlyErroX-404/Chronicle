...
function GraphViewer({ graph }: { graph: Graph }) {
  ...
  useEffect(() => {
    ...
    cy.current = cytoscape({
      ...
      style: [
        { selector: "node", style: { "background-color": "data(color)", label: "data(label)", color: "#e2e8f0", "font-size": "10px", "text-wrap": "wrap", "text-max-width": "110px", "text-valign": "bottom", "text-margin-y": 7, width: "30px", height: "30px" } },
        { selector: "edge", style: { width: 1.5, "line-color": "#475569", "target-arrow-color": "#475569", "target-arrow-shape": "triangle", label: "data(label)", color: "#94a3b8", "font-size": "9px", "text-rotation": "autorotate" } },
      ],
    });
...
export function ReportWorkbench() {
  ...
  const [message, setMessage] = useState("Submit a public threat report URL or PDF to begin analysis.");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const next = (await response.json()) as { id: string; status: string; progress?: string; partial?: boolean; error?: string };
      setJob(next);
      setMessage(next.status === "failed"
  ...
  const submit = async (event: FormEvent) => {
    ...
    const response = file
      ? await fetch("/api/v1/reports", { method: "POST", body: (() => { const data = new FormData(); data.set("file", file); return data; })() })
      : await fetch("/api/v1/reports", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ url }) });
    if (!response.ok) return setMessage(await apiError(response));
  ...
  return (
    <section className="workbench">
      ...
      {graph && <><div className="graph-heading"><h2>Extracted knowledge graph</h2><span>{graph.nodes.length} entities · {graph.edges.length} relationships</span></div><GraphViewer graph={graph} /></>}
    </section>
  );
}
