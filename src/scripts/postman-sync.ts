...
const main = async () => {
  ...
  const existingCollection = collections.find(
    (c: { name: string }) => c.name === collection.info.name,
  );
  const existingEnvironment = environments.find(
    (e: { name: string }) => e.name === environment.name,
  );

  if (existingCollection) {
    await apiCall('PUT', `/collections/${existingCollection.id}`, { collection });
    console.log(
      `[postman-sync] updated collection "${collection.info.name}" (${existingCollection.id})`,
    );
  } else {
    const created = await apiCall('POST', '/collections', { collection });
    console.log(
      `[postman-sync] created collection "${collection.info.name}" (${created.collection.id})`,
    );
  }
  ...
  if (existingEnvironment) {
    ...
    console.log(
      `[postman-sync] updated environment "${environment.name}" (${existingEnvironment.id})`,
    );
  } else {
    const created = await apiCall('POST', '/environments', { environment });
    console.log(
      `[postman-sync] created environment "${environment.name}" (${created.environment.id})`,
    );
  }
